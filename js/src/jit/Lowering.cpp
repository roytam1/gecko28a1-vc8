...
LIRGenerator::visitTest(MTest *test)
{
    ...
    if (opd->isCompare() && opd->isEmittedAtUses()) {
            ...
            comp->compareType() == MCompare::Compare_Undefined)
        {
            if (left->type() == MIRType_Object) {
                ...
                LEmulatesUndefinedAndBranch *lir =
                    new LEmulatesUndefinedAndBranch(comp, useRegister(left), ifTrue, ifFalse, temp());
                return add(lir, test);
            }
            ...
            LIsNullOrLikeUndefinedAndBranch *lir =
                new LIsNullOrLikeUndefinedAndBranch(comp, ifTrue, ifFalse, tmp, tmpToUnbox);
        ...
        if (comp->compareType() == MCompare::Compare_Boolean) {
            ...
            LCompareBAndBranch *lir = new LCompareBAndBranch(comp, rhs, ifTrue, ifFalse);
            ...
            comp->compareType() == MCompare::Compare_Object)
        {
            ...
            LCompareAndBranch *lir = new LCompareAndBranch(comp, op, lhs, rhs, ifTrue, ifFalse);
            return add(lir, test);
        }
        ...
        if (comp->isFloat32Comparison()) {
            LAllocation lhs = useRegister(left);
            LAllocation rhs = useRegister(right);
            LCompareFAndBranch *lir = new LCompareFAndBranch(comp, lhs, rhs, ifTrue, ifFalse);
            return add(lir, test);
        }

        // Compare values.
        if (comp->compareType() == MCompare::Compare_Value) {
            LCompareVAndBranch *lir = new LCompareVAndBranch(comp, ifTrue, ifFalse);
