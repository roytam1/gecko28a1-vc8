...
void PeerConnectionImpl::GetStats_s(
  ...
  if (mMedia) {
    ...
    if (mediaStream) {
      ...
      for (std::vector<NrIceCandidatePair>::const_iterator p = candPairs.begin(); p != candPairs.end(); ++p) {
...
void PeerConnectionImpl::OnGetLogging_m(const std::string& pattern,
  ...
  if (!logging.empty()) {
    ...
    for (std::deque<std::string>::const_iterator l = logging.begin(); l != logging.end(); ++l) {
